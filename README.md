...
> **Important:** The JSON structure generated by export tools is not guaranteed to match the Terraform provider schema. Before using exported data in Terraform resource, verify that all fields and attributes match the Terraform provider documentation. Unsupported attributes can be silently ignored during deployment.
